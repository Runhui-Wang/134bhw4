    ...
    this.DisplayAll = function() {
        ...
        if (this.blogs.length > 0) {
            ...
            for (i=0; i < this.blogs.length; i++) {
                //data += '<li>';
                data += '<b>Title</b>: '+this.blogs[i] 
                data += '<br>'
                data += ' <b>Date</b>:' + this.times[i] 
                data += '<br>'
                data += ' <b>Summary</b>: ' + this.summarys[i];
                data += '<br>'
                data += '<button id="edit" onclick="crud.Retreive(' + i + ')">✏️ Edit</button>';
                data += '<button id="delete" onclick="crud.Remove(' + i + ')">🗑️Delete</button>' + '<br>';
                //data += '</li>';
            }
        }
        else {
